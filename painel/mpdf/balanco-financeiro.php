...
$mensalidadesabertas = $totalRows_msm - $totalRows_msmp;
...
$sqdtpl = "SELECT * FROM rfa_pagar WHERE clube='$clube' AND MONTH(data_pagar) = '$filtromes' AND YEAR(data_pagar) = '$filtroano'";
...
$sqmtg2 = "SELECT SUM(valor_mensalidade) as valor FROM rfa_mensalidades WHERE clube='$clube' AND pagamento=1";
$mtgtotal2 = mysqli_query($link, $sqmtg2) or die(mysqli_error($link));
$row_mtgtotal2 = mysqli_fetch_assoc($mtgtotal2);
...
$sqrecpg2 = "SELECT SUM(valor_receita) as valor FROM rfa_receitas WHERE clube='$clube' AND status_receita=2";
$recpgtotal2 = mysqli_query($link, $sqrecpg2) or die(mysqli_error($link));
$row_recpgtotal2 = mysqli_fetch_assoc($recpgtotal2);
...
$sqdtg2 = "SELECT SUM(valor_pagar) as valor FROM rfa_pagar WHERE clube='$clube' AND MONTH(data_pagar) <= '$filtromes' AND YEAR(data_pagar) <= '$filtroano' AND status_pagar=2";
$despesatotalg2 = mysqli_query($link, $sqdtg2) or die(mysqli_error($link));
$row_despesatotalg2 = mysqli_fetch_assoc($despesatotalg2);
...
$sqtxg2 = "SELECT SUM(taxa) as valor FROM rfa_mensalidades WHERE clube='$clube' AND MONTH(data_pagamento) <= '$filtromes' AND YEAR(data_pagamento) <= '$filtroano' AND pagamento=1";
$txtotalg2 = mysqli_query($link, $sqtxg2) or die(mysqli_error($link));
$row_txtotalg2 = mysqli_fetch_assoc($txtotalg2); 
...
$sqfng2 = "SELECT SUM(valor_fundo) as valor FROM rfa_fundos WHERE clube='$clube' AND status_fundo=2";
$fntotalg2 = mysqli_query($link, $sqfng2) or die(mysqli_error($link));
$row_fntotalg2 = mysqli_fetch_assoc($fntotalg2);
...
$totalgeralk = ($row_mtgtotal2['valor'] + $row_recpgtotal2['valor'] + $row_sldtotal['valor']) - ($row_despesatotalg2['valor'] + $row_txtotalg2['valor'] + $row_fntotalg2['valor']);
$totalgeral = ($row_mtgtotal1['valor'] + $row_recpgtotal1['valor'] + $row_sldtotal['valor']) - ($row_despesatotalg1['valor'] + $row_txtotalg1['valor']);
...
$somasaldo = ($fundogeral + $totalgeralk);

 $html = "
 <fieldset>

 <h1>Balanço Financeiro da ".$row_pegaclube['nome_clube']."</h1>
...
<table cellpadding='0' cellspacing='0'>
 	...
 	<tr>
 		<td width='200' style='border: 1px solid #000; border-collapsed: collapsed; font-weight: bold; padding: 2px'>Qtd. de associados:</td>
 	...
 	<tr>
 		...
 		<td style='border: 1px solid #000; border-collapsed: collapsed; padding: 2px'>".$mensalidadesabertas." mensalidades</td>
 	</tr>
 	<tr>
